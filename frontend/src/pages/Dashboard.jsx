...
const DEMO_DATA = {
  resumes: [
    {
      _id: "demo-1",
      fileName: "Sameer_Patil_Resume.pdf",
      predictedRole: "Senior Full Stack Developer",
      atsScore: 85,
      skillsDetected: [
        "React",
        "Node.js",
      ...
      aiInsights: {
        summary:
          "Strong background in full-stack development with expertise in modern JavaScript frameworks.",
        questions: [
          "Explain React hooks",
    ...
    {
      _id: "demo-2",
      fileName: "Sameer_Patil_Resume_v2.pdf",
      predictedRole: "Senior Full Stack Developer",
      atsScore: 92,
      skillsDetected: [
        "React",
      ...
      aiInsights: {
        summary:
          "Updated resume with Next.js experience, showing continuous learning.",
        questions: [
          "Explain React hooks",
          "Describe REST vs GraphQL",
          "How do you optimize MongoDB queries?",
...
const Dashboard = ({ demoMode = false }) => {
  ...
  const userName = demoMode
    ? "Guest"
    : getCurrentUser()?.user?.name?.split(" ")[0] || "there";

  const [loading, setLoading] = useState(!demoMode);
  const [resumes, setResumes] = useState(demoMode ? DEMO_DATA.resumes : []);
  const [latestResume, setLatestResume] = useState(
    demoMode ? DEMO_DATA.resumes[0] : null
  );
  const [topMatch, setTopMatch] = useState(
    demoMode ? DEMO_DATA.topMatch : null
  );
  const [aiStatus, setAiStatus] = useState(
    demoMode ? DEMO_DATA.aiStatus : { processing: false, ready: false, questionCount: 0 }
  );
    ...
    <div className="max-w-6xl mx-auto space-y-5 pb-8">
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            type="button"
            onClick={() => handleProtectedAction("/app/job-matching")}
            className="inline-flex items-center gap-2 text-sm font-semibold text-zinc-700 px-4 py-2.5 rounded-xl border border-zinc-200 bg-white hover:border-indigo-300 hover:bg-indigo-50/50 hover:-translate-y-0.5 transition-all duration-200 shadow-sm"
          >
          ...
          <button
            type="button"
            onClick={() => handleProtectedAction("/app/upload")}
            className="inline-flex items-center gap-2 text-sm font-semibold text-white px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 hover:-translate-y-0.5 transition-all duration-200 shadow-md shadow-indigo-500/25"
          >
      ...
      >
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button
            type="button"
            onClick={() =>
              handleProtectedAction("/app/insights")
            }
      ...
      <RecentActivity
        ...
        onViewAll={() => handleProtectedAction("/app/history")}
      />
    </div>
  );
};
